	...
	<div class="form-body">
		...
		<div class="row top-border">
				...
				<div class="col-md-5">
					<select class="form-control select2me calc" name="categories[]" id="categories[]" multiple placeholder="選択..." style="min-width:100px;">
						...
						@foreach($categories as $key=>$category)
							@if ((is_object(old('categories')) && count(get_object_vars(old('categories'))) > 0) || (is_array(old('categories')) && count(old('categories'))) > 0)
								<option value="{{ $category->id }}" @if (in_array((string)$category->id,  old('categories'))) selected @endif>{{$category->name }}</option>
				...
				<div class="col-md-5">
					<select name="recommend" class="form-control select2me calc"  placeholder="選択..." style="height:33px !important">
						...
						@foreach(config('consts')['BOOK']['RECOMMEND'] as $key=> $recommend)
							<option value="{{$key}}" @if ($key==old('recommend')) selected @endif @if(isset($book->recommend)&&($book->recommend==$key)) selected @endif>{{$recommend['TITLE']}}</option>
						@endforeach
					</select>
			...
			<div class="col-md-12  margin-bottom-5 {{ $errors->has('url') ? ' has-danger' : '' }}">
				...
				<div class="col-md-5">
					<textarea id="honto_url" name="honto_url" rows="3" style="width: 100%">{{(isset($book)? $book->honto_url: '')}}</textarea>
				</div>
			</div>				
			<div class="col-md-12  margin-bottom-5 {{ $errors->has('url') ? ' has-danger' : '' }}">
				<label class="control-label col-md-7 text-md-left"><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;楽天ブックスで見る。</strong></label>
				<div class="col-md-5">
					<textarea id="seven_net_url" name="seven_net_url" rows="3" style="width: 100%">{{(isset($book)? $book->seven_net_url: '')}}</textarea>
				</div>
			</div>	
			@else
			<div class="col-md-12  margin-bottom-5">
			...
			<div class="col-md-12  margin-bottom-5 {{ $errors->has('register_visi_type') ? ' has-danger' : '' }}">
				...
				<div class="form-group col-md-5">					
					...
					<select class="bs-select form-control" name="register_visi_type" placeholder="選択.." style="height:33px !important">
						@for($i=1; $i<3; $i++)
							<option value="{{$i - 1}}" @if(old('register_visi_type')!='') @if($i-1==old('register_visi_type')) selected @endif @else @if(isset($book)) @if($book->register_visi_type == $i-1) selected @endif @endif @endif>{{config('consts')['BOOK']['REGISTER_VISI_TYPE'][$i]}}</option>
						@endfor
					</select>
